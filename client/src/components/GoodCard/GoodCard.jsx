...
export const GoodCard = (good ) =>{
  ...
  <div className={styles.goodCard}>
    ...
    <Link className={styles.goodCard__link} to={`/${_id}`} >
				<div className={styles.goodCard__imageContainer}>
				  {picture && <img className={styles.goodCard__image} src={host+ picture} alt={name} />}
				</div>
    ...
    <p className={styles.goodCard__name}>{name}</p>
        ...
        <div className= {styles.goodCard__buy}>
          <div className= {styles.goodCard__price}>{priceFormating(price)} <span  className= {styles.goodCard__currency} >ГРН</span> 
          </div>
          <GoodBuy good = {good}  />
          {/* <Button className={styles.goodCard__button} >Купить </Button> */}
        </div >
  </div>
)};
